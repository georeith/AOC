...
function inputToCoordinates(input) {
  ...
  for (const command of commands) {
    const [, direction, amount] = command.match(/([LRDU])(\d+)/);
    for (let z = 0; z < amount; z++) {
      const [x, y] = coordinates[coordinates.length - 1] || [0, 0];
      coordinates.push(travel[direction](x, y));
    }
  }
  return coordinates;
}
...
function coordinatesToString(coordinates) {
  return coordinates.map(coord => coord.toString());
}

function getIntersections(input1, input2) {
  const coordinates1 = coordinatesToString(inputToCoordinates(input1));
  const coordinates2 = coordinatesToString(inputToCoordinates(input2));
  const coordinateSet2 = new Set(coordinates2);
  const intersections = coordinates1.filter(coordinate =>
    coordinateSet2.has(coordinate)
  );

  return { coordinates1, coordinates2, intersections };
}

function closestIntersection(input1, input2) {
  const { intersections } = getIntersections(
    input1,
    input2
  );
  const manhattanDistances = intersections.map(stringIntersection =>
    stringIntersection.split(",").map(stringCoord => parseInt(stringCoord, 10))
  ).map(
...
function quickestIntersection(input1, input2) {
    const { coordinates1, coordinates2, intersections } = getIntersections(input1, input2);
    const steps = intersections.map(intersection => coordinates1.indexOf(intersection) + coordinates2.indexOf(intersection));
    return Math.min(...steps) + 2;
}